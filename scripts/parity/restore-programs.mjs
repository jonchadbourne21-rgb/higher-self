...
try {
  ...
  if(opts['--apply']){
    const [[lock]]=await conn.execute('SELECT GET_LOCK(?, 10) AS acquired',[LOCK_NAME]);
...
finally{if(locked&&conn)await conn.execute('SELECT RELEASE_LOCK(?)',[LOCK_NAME]).catch(()=>{});if(conn)await conn.end().catch(()=>{});}
