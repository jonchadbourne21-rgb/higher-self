...
const UNAUTHED_SIGNALS = ["Please login (10001)", "UNAUTHORIZED", "Missing session cookie", "Session expired", "jwt expired", "invalid signature"];

function isAuthError(error: Error | null): boolean {
  if (!error) return false;
  const msg = error.message || error.toString();
  return UNAUTHED_SIGNALS.some(signal => msg.includes(signal));
}

interface Props { children: ReactNode }
interface State { hasError: boolean; error: Error | null; isAuthError: boolean }
...
class ErrorBoundary extends Component<Props, State> {
  ...
  render() {
    ...
    if (this.state.isAuthError) {
      redirectToLogin("/");
      return <div className="flex items-center justify-center min-h-screen bg-background"><p className="text-muted-foreground text-sm">Redirecting to sign in…</p></div>;
    }
