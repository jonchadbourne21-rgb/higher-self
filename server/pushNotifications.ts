...
// ── Calendar event reminders (1 hour before) ─────────────────────────────────
// Runs every 5 minutes, checks which users have calendar events in the next
// 1 hour window, and sends a reminder push notification.

let calendarReminderInterval: ReturnType<typeof setInterval> | null = null;

export function startCalendarReminderScheduler() {
  if (calendarReminderInterval) return; // already running

  console.log("[Push] Calendar event reminder scheduler started");

  calendarReminderInterval = setInterval(async () => {
    try {
      const { getUpcomingEvents, getAllActivePushSubscriptions } = await import("./db");
      const subscriptions = await getAllActivePushSubscriptions();
      if (subscriptions.length === 0) return;
      ...
      for (const sub of subscriptions) {
        try {
          ...
          const upcomingEvents = await getUpcomingEvents(sub.userId, 10);

          // Filter events that fall in the next 1-hour window
          const remindableEvents = upcomingEvents.filter((event) => {
            const eventTime = new Date(event.eventDate);
            // Send reminder if event is between now + 55 min and now + 65 min
            // (to avoid duplicate sends, only send once in a 10-minute window)
            const minutesTillEvent = (eventTime.getTime() - now.getTime()) / 60000;
            return minutesTillEvent >= 55 && minutesTillEvent <= 65;
          });

          if (remindableEvents.length === 0) continue;

          // Send a reminder for the first upcoming event
          const event = remindableEvents[0];
          const eventTime = new Date(event.eventDate);
          ...
          if (ok) {
            console.log(
              `[Push] Sent calendar reminder to user ${sub.userId} for event "${event.title}"`
            );
          }
        } catch (innerErr) {
          console.error(`[Push] Calendar reminder error for user ${sub.userId}:`, innerErr);
        }
      }
    } catch (err) {
      console.error("[Push] Calendar reminder scheduler error:", err);
    }
  }, 5 * 60 * 1000); // check every 5 minutes
}

export function stopCalendarReminderScheduler() {
  if (calendarReminderInterval) {
    clearInterval(calendarReminderInterval);
    calendarReminderInterval = null;
  }
}
