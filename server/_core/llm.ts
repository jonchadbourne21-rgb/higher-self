...
function requireOpenAIConfig() {
  ...
  return { apiKey: ENV.openaiApiKey, model: ENV.openaiModel || "gpt-4.1-mini" };
}

export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  const api = requireOpenAIConfig();
  const {
    messages,
    tools,
    toolChoice,
  ...
  if (!response.ok) {
    ...
    const usageLimited = response.status === 429 || /quota|rate.?limit|usage/i.test(providerText);
    if (usageLimited) {
      const error = new Error("AI service temporarily unavailable. Please try again later.");
      (error as Error & { code?: string }).code = "USAGE_EXHAUSTED";
      throw error;
    }
    const error = new Error(`AI service request failed (${response.status}).`);
    (error as Error & { code?: string }).code = "LLM_PROVIDER_ERROR";
    throw error;
  }
